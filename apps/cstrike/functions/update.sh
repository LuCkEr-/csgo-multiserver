...
App::isUpToDate () {
	...
	# variables
	local APPMANIFEST="$INSTALL_DIR/steamapps/appmanifest_232330.acf"
...
App::performUpdate () {
	...
	cat <<-EOF > "$STEAMCMD_SCRIPT"
		...
		app_update 232330 $( [[ $ACTION == repair ]] && echo "validate" )
		quit
	EOF
	...
	while (( $code && ++try <= tries )); do
		...
		egrep "Success! App '232330'.*(fully installed|up to date)" \
