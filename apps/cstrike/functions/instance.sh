...
App::finalizeInstance () (
	...
	# create cstrike directory
	mkdir -p $INSTANCE_DIR/cstrike
)
...
App::applyInstancePermissions () {
	...
	chmod o-r "$INSTANCE_DIR/cstrike/cfg/autoexec.cfg"
	chmod o-r "$INSTANCE_DIR/cstrike/cfg/server.cfg"
} 2>/dev/null
